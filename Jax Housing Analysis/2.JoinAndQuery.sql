...
SELECT  
		...
		/* Parcel table query */
		CASE WHEN mailing_address_2 NOT NULL THEN (TRIM(mailing_address_1)||' '||TRIM(mailing_address_2)) 
			ELSE TRIM(mailing_address_1) END AS mailing_address,
		/* Two properties in the database did not list mailing_city. In both cases, the owner resides in Jacksonville. */
		CASE WHEN city NOT NULL THEN TRIM(city)
			ELSE 'JACKSONVILLE' END AS mailing_city, 
		IFNULL(state, 'FOREIGN COUNTRY') AS mailing_state, property_use, 
		IFNULL(subdivision_name, 'NOT LISTED') AS subdivision, neighborhood, value_method, 
		CASE WHEN cap_base_yr = 1 THEN 0
			ELSE cap_base_yr END AS cap_base_yr, 
		market_value, assessed_value, building_value, just_value, school_taxable, county_taxable, sjrwmd_taxable, 
		tax_district, lot_sf,
...
(SELECT  RE,
		...
		MAX(CASE WHEN ft_descr LIKE 'Boat%' THEN units
			ELSE NULL END) AS boat_cv,
		MAX(CASE WHEN ft_descr LIKE 'Boat%' THEN ppu
			ELSE NULL END) AS boatcv_ppu,
		MAX(CASE WHEN ft_descr LIKE 'Boat%' THEN actual_yr_built
			ELSE NULL END) AS boatcv_yr,
		MAX(CASE WHEN ft_descr LIKE 'Carport%' THEN units
			ELSE NULL END) AS carport,
		MAX(CASE WHEN ft_descr LIKE 'Carport%' THEN ppu
			ELSE NULL END) AS carport_ppu,
		MAX(CASE WHEN ft_descr LIKE 'Carport%' THEN actual_yr_built
			ELSE NULL END) AS carport_yr,
		MAX(CASE WHEN ft_descr = 'Cov Patio' THEN units
			ELSE NULL END) AS cov_patio,
		MAX(CASE WHEN ft_descr = 'Cov Patio' THEN ppu
			ELSE NULL END) AS covpatio_ppu,
		MAX(CASE WHEN ft_descr = 'Cov Patio' THEN actual_yr_built
			ELSE NULL END) AS covpatio_yr,
		MAX(CASE WHEN ft_descr = 'Deck Wd' THEN units
			ELSE NULL END) AS deck,
		MAX(CASE WHEN ft_descr = 'Deck Wd' THEN ppu
			ELSE NULL END) AS deck_ppu,
		MAX(CASE WHEN ft_descr = 'Deck Wd' THEN actual_yr_built
			ELSE NULL END) AS deck_yr,
		MAX(CASE WHEN ft_descr LIKE 'Dock%' THEN units
			ELSE NULL END) AS dock,
		MAX(CASE WHEN ft_descr LIKE 'Dock%' THEN ppu
			ELSE NULL END) AS dock_ppu,
		MAX(CASE WHEN ft_descr LIKE 'Dock%' THEN actual_yr_built
			ELSE NULL END) AS dock_yr,
		MAX(CASE WHEN ft_descr LIKE 'Firep%' THEN units
			ELSE NULL END) AS fireplace,
		MAX(CASE WHEN ft_descr LIKE 'Firep%' THEN ppu
			ELSE NULL END) AS fireplace_ppu,
		MAX(CASE WHEN ft_descr LIKE 'Firep%' THEN actual_yr_built
			ELSE NULL END) AS fireplace_yr,
		MAX(CASE WHEN ft_descr = 'Pool' THEN units
			ELSE NULL END) AS pool,
		MAX(CASE WHEN ft_descr = 'Pool' THEN ppu
			ELSE NULL END) AS pool_ppu,
		MAX(CASE WHEN ft_descr = 'Pool' THEN actual_yr_built
			ELSE NULL END) AS pool_yr,
		MAX(CASE WHEN ft_descr = 'Scr Porch' THEN units
			ELSE NULL END) AS scr_porch,
		MAX(CASE WHEN ft_descr = 'Scr Porch' THEN ppu
			ELSE NULL END) AS scrporch_ppu,
		MAX(CASE WHEN ft_descr = 'Scr Porch' THEN actual_yr_built
			ELSE NULL END) AS scrporch_yr,
		MAX(CASE WHEN ft_descr LIKE 'Screen%' THEN units
			ELSE NULL END) AS screen_encl,
		MAX(CASE WHEN ft_descr LIKE 'Screen%' THEN ppu
			ELSE NULL END) AS screenencl_ppu,
		MAX(CASE WHEN ft_descr LIKE 'Screen%' THEN actual_yr_built
			ELSE NULL END) AS screenencl_yr,
		MAX(CASE WHEN ft_descr LIKE 'Shed%' THEN units
			ELSE NULL END) AS shed,
		MAX(CASE WHEN ft_descr LIKE 'Shed%' THEN ppu
			ELSE NULL END) AS shed_ppu,
		MAX(CASE WHEN ft_descr LIKE 'Shed%' THEN actual_yr_built
			ELSE NULL END) AS shed_yr,
		MAX(CASE WHEN ft_descr = 'Sun Room' THEN units
			ELSE NULL END) AS sun_rm,
		MAX(CASE WHEN ft_descr = 'Sun Room' THEN ppu
			ELSE NULL END) AS sunrm_ppu,
		MAX(CASE WHEN ft_descr = 'Sun Room' THEN actual_yr_built
...
(SELECT  RE,
		IFNULL(building,0) AS building,
		SUM(CASE WHEN substructure_descr = 'Addition' THEN 1
			ELSE NULL END) AS additions,
		SUM(CASE WHEN substructure_descr = 'Addition' THEN effec_area
			ELSE NULL END) AS additions_effec_area,
		SUM(CASE WHEN substructure_descr = 'Balcony' THEN effec_area
			ELSE NULL END) AS balcony_effec_area,
		SUM(CASE WHEN substructure_descr LIKE '%Garage%' THEN effec_area
			ELSE NULL END) AS garage_effec_area,
		SUM(CASE WHEN substructure_descr = 'Finished Storage' THEN effec_area
			ELSE NULL END) AS fin_storage_effec_area,
		SUM(CASE WHEN substructure_descr = 'Unfinished Storage' THEN effec_area
...
WHERE property_use = '0100' AND ((status = 'Qualified' AND improved = 'I') OR status IS NULL)